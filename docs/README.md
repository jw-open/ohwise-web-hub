...
| 3 | [03-FRONTEND-IMPLEMENTATION-GUIDE.md](03-FRONTEND-IMPLEMENTATION-GUIDE.md) | Step-by-step: build the site in ohwise-web-hub (blog first, sitemap). **Design:** simple, official, industrial, Apple-style; study [Claude Code](https://claude.com/product/claude-code), [Manus](https://manus.im/), [OpenAI](https://openai.com/) for landing page inspiration. |

**Scope:** Blog only at `/blog` and `/blog/[slug]` (single level; no thoughts/product/content-by-type). Include About, Contact, Privacy, Terms, **social links section** (footer/header), and optionally **video tutorials on the landing page**. Strapi backend: **strapi.ohwise.com**. **Not in scope:** content comments and OAuth (project owner will add if needed). Pricing and documentation can be added later. **Database:** Production **MySQL** is on AWS EC2 (already set up); use local MySQL for dev. **CI/CD:** Will be set up by the project owner after the backend and frontend backbone are done. Schema in doc 02 is **suggested** — add/remove/rename fields as you see fit; keep the API contract. **Naming:** See 02 §1.1 for URL paths, API endpoints, and Strapi content-type names.
